"""
Scraper para valdizarbe.es (PrestaShop search_ajax).
"""
...
RE_URL = re.compile(r'href="(https://valdizarbe\.es/(\d+)-([^"]+)\.html)"')
...
RE_IMG = re.compile(r'src="(https://valdizarbe\.es/\d+-[^"]+\.jpg)"')
RE_VEHICULOS = [
    re.compile(rf'({marca}\s+[A-Z0-9\s\(\)\.IVX-]+)', re.IGNORECASE)
    for marca in [
        r'CITRO[ËE]N', 'PEUGEOT', 'SEAT', 'HYUNDAI', 'KIA', 'FORD',
        'VOLKSWAGEN', 'RENAULT', 'OPEL', 'BMW', 'AUDI', 'MERCEDES',
        'TOYOTA', 'HONDA', 'MAZDA', 'NISSAN', 'DACIA', 'FIAT', 'SKODA', 'VOLVO',
    ]
]
...
class ValdizarbeScraper(DesguaceScraper):
    id = "valdizarbe"
    nombre = "Valdizarbe"
    url_base = "https://valdizarbe.es"

    def buscar(self, referencia: str) -> List[Dict]:
        try:
            return self._buscar_todas_paginas(referencia)
        except Exception as e:
            logger.error(f"[Valdizarbe] Error buscando {referencia}: {e}")
            return []
